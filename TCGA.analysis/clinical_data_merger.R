...
dataframes <- list(
  load_follow_up_data('./data/data_follow_up_v1.5_brca.txt'),
  load_follow_up_data('./data/data_follow_up_v2.1_brca.txt'),
  load_follow_up_data('./data/data_follow_up_v4.0_nte_brca.txt')
)

follow_ups <- merge_follow_ups(dataframes)
follow_ups <- clean_follow_up_dataframe_data(follow_ups)

clinical_data <- load_clinical_data('./data/data_clinical_raw.txt')

clinical_data <- clean_clinical_data(clinical_data)

clinical_data_followup = attach_followup_data(clinical_data, follow_ups)

save_clinical_data(clinical_data_followup, './output/data_clinical_followup.txt')
...
save_clinical_data(clinical_data_redux, './output/data_clinical_followup_redux.txt')
