################################################################################
# This scripts generates a Cox PH Model of the relationship of MKI67 gene 
...
expression_data <- load_gene_expression_data('./data/data_expression_genes_filtered_mki67.txt')

patient_data_fields <- append(BASIC_SURVIVAL_FIELDS, 'MKI67')
...
#Event  -> Death
#Factor -> HMMR
summary(coxph(death.surv ~ MKI67 , data=brca_patient_data))

#Event  -> Recur
#Factor -> HMMR

summary(coxph(recur.surv ~ MKI67 , data=brca_patient_data))
